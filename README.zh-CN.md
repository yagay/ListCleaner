...
签名配置及自动发布流程见 [Release 构建说明](docs/RELEASE.zh-CN.md)。不要提交私钥、签名密码或 `local.properties`。

main 分支的源码或构建配置变更会自动编译 Debug 并运行单元测试，也可在 Actions 手动运行。Debug APK 可从对应运行的 Artifacts 下载。正式发布请使用 Release 工作流；已有版本不会重复发布，新版本需要同时更新版本名和版本码。

主要源码位置：

- `app/src/main/java/com/yagay/ListCleaner/ui`：界面与交互。
- `app/src/main/java/com/yagay/ListCleaner/data`：目录扫描、配置存储、Root 操作与诊断。
- `app/src/main/java/com/yagay/ListCleaner/domain`：规则、排序及校验逻辑。
- `app/src/main/java/com/yagay/ListCleaner/xposed`：模块入口与系统 Hook。
- `app/src/test`、`tools/*Check.java`：单元测试与宿主 Java 回归检查。

反馈问题时，请附上系统版本、设备型号、框架版本、问题分类和复现步骤；有必要时附经过检查的诊断日志。
