...
每个页面都可以独立设置**批量操作锁**。锁定只保护“全选、反选等批量操作”：整体锁定会保护整个应用，部分锁定只保护指定子项；锁定项仍可手动单独修改。锁定状态不会参与排序，也不会改变“已选 / 半选 / 未选”的计算；“已锁定”筛选可集中查看整体或部分锁定的应用。
...
组件页同样支持批量操作锁和“已锁定”筛选。这里的锁仅用于保护全选、反选等批量操作，**不等于 Root 组件禁用状态**；单项仍可手动修改。
