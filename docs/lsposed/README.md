...
- **批量操作锁**：规则、排序和组件页共用锁定机制；整体或部分锁定的项目会跳过全选/反选等批量操作，仍可手动修改。“已锁定”可单独筛选；锁定不参与排序，也不改变已选/半选/未选状态。
...
规则页的应用勾选只针对当前分类和搜索条件下显示的组件，不会禁用整个应用。组件页则实际修改系统组件启用状态，两者作用不同。锁图标只用于批量操作保护：被锁项目跳过全选/反选，但仍能手动修改；每个页面的锁定状态独立保存。
